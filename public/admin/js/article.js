...
(function () {
  ...
  buttons.forEach(function (button) {
    button.addEventListener("click", function (e) {
      ...
      fetch("/admin/articles/change-status/" + newStatus + "/" + id, {
        method: "PATCH",
...
(function () {
  ...
  batchItems.forEach(function (item) {
    item.addEventListener("click", function () {
      ...
      var confirmMsg = "Bạn có chắc muốn thực hiện hành động này cho " + ids.length + " bài viết?";
      if (action === "delete") {
        confirmMsg = "Bạn có chắc muốn xoá " + ids.length + " bài viết đã chọn?";
      }

      if (!confirm(confirmMsg)) return;

      fetch("/admin/articles/change-multi", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
