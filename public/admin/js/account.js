...
(function () {
  ...
  batchItems.forEach(function (item) {
    item.addEventListener("click", function () {
      ...
      var confirmMsg = "Bạn có chắc muốn thực hiện hành động này cho " + ids.length + " tài khoản?";
      if (action === "delete") {
        confirmMsg = "Bạn có chắc muốn xoá " + ids.length + " tài khoản đã chọn?";
      }

      if (!confirm(confirmMsg)) return;

      fetch("/admin/accounts/change-multi", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
